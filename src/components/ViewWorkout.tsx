...
type TUpdate = { Update: any, workoutName: string, exercises: TWorkout[] };
const ViewWorkout = () => {
    const {user} = useAuth();
    const navigate = useNavigate();
    const [dataLoaded, setDataLoaded] = useState(false);
    const [data, setData] = useState<TWorkout>();
    ...
    const setExerciseName = (name: string, index: number) => {
        // const newExercies =
        // setData({
        //     ...data,
        //     exercises:
        ...
        return () => {
            async function getData() {
                try {
                    ...
                    const {workoutName, workoutType, exercises} = workouts[0]
                    // @ts-ignore
                    setData({workoutName, workoutType, exercises})
                    setDataLoaded(true);
                } catch (error: any) {
    ...
    const {workoutType, workoutName, exercises} = data;
    console.table(data)
    return (
        ...
        rounded-lg shadow-md"
              ...
              {workoutType}
                    ...
                    {exercises.map((exercise: TExercise) => {
                        return (
                            <Exercise ex={exercise} workoutType={workoutType} workoutName={workoutName}
                                      editMode={editMode}/>
                        )
                    })}
                    {/*              <div*/}
...
type TExerciseProps = {
    ...
    workoutName: string,
    editMode: boolean
    setData: SetStateAction<TWorkout>
}

const Exercise: React.FC<TExerciseProps> = ({ex, workoutType, workoutName, editMode}) => {
    const {exercise, sets, reps, weight} = ex;
    return (
        <>
            <div
                className="mt-10 p-8 rounded-md flex flex-col item-center
